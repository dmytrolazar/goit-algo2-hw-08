...
class ThrottlingRateLimiter:
    ...
    def can_send_message(self, user_id: str) -> bool:
        """Перевіряє, чи можна відправити повідомлення, ґрунтуючись на останньому часі відправлення."""
        current_time = time.time()
        last_time = self.user_last_message_time.get(user_id, 0)
        return (current_time - last_time) >= self.min_interval

    def record_message(self, user_id: str) -> bool:
        """Реєструє повідомлення користувача, якщо дозволено."""
    ...
    def time_until_next_allowed(self, user_id: str) -> float:
        """Розраховує, скільки часу залишилося до можливості відправлення наступного повідомлення."""
        current_time = time.time()
        last_time = self.user_last_message_time.get(user_id, 0)
